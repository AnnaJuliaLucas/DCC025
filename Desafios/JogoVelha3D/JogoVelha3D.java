...
public class JogoVelha3D {
    ...
    public JogoVelha3D() {
        ...
        this.cubo = new char[JogoVelha3D.DIMENSAO][JogoVelha3D.DIMENSAO][JogoVelha3D.DIMENSAO];

        for (int i = 0; i < JogoVelha3D.DIMENSAO; i++) {
            for (int j = 0; j < JogoVelha3D.DIMENSAO; j++) {
                for (int k = 0; k < JogoVelha3D.DIMENSAO; k++) {
                    this.cubo[i][j][k] = '-';
                }
            }
        }
    ...
    private boolean verificaJogadaVitoriosa(int[] posicao) {
        ...
        for (int i = 0; i < JogoVelha3D.DIMENSAO; i++) {
            if (!(i == linha && i == coluna)) {
                char simboloIteracao = this.cubo[camada][i][JogoVelha3D.DIMENSAO - i - 1];
                if (simbolo != simboloIteracao) {
                    formaLinha = false;
                }
            }
        ...
        for (int i = 0; i < JogoVelha3D.DIMENSAO; i++) {
            if (!(i == camada && i == coluna)) {
                char simboloIteracao = this.cubo[i][linha][JogoVelha3D.DIMENSAO - i - 1];
                if (simbolo != simboloIteracao) {
                    formaLinha = false;
                }
            }
        ...
        for (int i = 0; i < JogoVelha3D.DIMENSAO; i++) {
            if (!(i == camada && i == linha && i == coluna)) {
                char simboloIteracao = this.cubo[i][i][JogoVelha3D.DIMENSAO - i - 1];
                if (simbolo != simboloIteracao) {
                    formaLinha = false;
                }
            }
        ...
        for (int i = 0; i < JogoVelha3D.DIMENSAO; i++) {
            if (!(i == camada && i == linha && i == coluna)) {
                char simboloIteracao = this.cubo[i][JogoVelha3D.DIMENSAO - i - 1][i];
                if (simbolo != simboloIteracao) {
                    formaLinha = false;
                }
            }
        ...
        for (int i = 0; i < JogoVelha3D.DIMENSAO; i++) {
            if (!(i == camada && i == linha && i == coluna)) {
                char simboloIteracao = this.cubo[JogoVelha3D.DIMENSAO - i - 1][i][i];
                if (simbolo != simboloIteracao) {
                    formaLinha = false;
                }
            }
    ...
    private boolean validaIndice(int indice) {
        return (indice >= 0 && indice < JogoVelha3D.DIMENSAO);
    }
    ...
    private int[] converteJogada(String[] argumentos) {
        ...
        int[] posicao = new int[JogoVelha3D.DIMENSAO];
    ...
    private int[] interpretaJogada(String jogada) {
        if (jogada.startsWith("(") && jogada.endsWith(")")) {
            ...
            if (comandos.length == JogoVelha3D.DIMENSAO) {
    ...
    private int[] obtemJogadaDoBot() {
        ...
        do {
            for (int i = 0; i < JogoVelha3D.DIMENSAO; i++) {
                jogada[i] = aleatorio.nextInt(JogoVelha3D.DIMENSAO);
            }
    ...
    private void interfaceJogo() {
        ...
        int posicoesMaximas = (int) Math.pow(JogoVelha3D.DIMENSAO, 3);
        int[] posicao = null;
        int turno = 0;
    ...
    public static void main(String[] args) {
        JogoVelha3D jogo = new JogoVelha3D();
        jogo.iniciar();
    }
}
